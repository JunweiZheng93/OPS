...
from ..dcnv3.modules import DCNv4
...
class CrossAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.q = nn.Linear(out_dim, all_head_dim, bias=False)
...
class SideAdapterNetwork(nn.Module):
            ...
            for i in range(len(fusion_index)):
                ca_clips.append(
                    CrossAttention(
                        ...
                        attn_head_dim=None,
                        out_dim=embed_dims))
            self.ca_clips = nn.ModuleList(ca_clips)
        else:
    ...
    def fuse_clip_ca(self, fused_index: int, x: torch.Tensor, clip_feature: torch.Tensor, hwshape: Tuple[int, int], L: int):
        """Fuse CLIP feature and visual tokens."""
        k = rearrange(clip_feature, 'N C H W -> N (H W) C').contiguous()
        v = rearrange(clip_feature, 'N C H W -> N (H W) C').contiguous()
        fused_clip = self.ca_clips[fused_index](x, k, v)
        return fused_clip
...
class OPSCLIPHead(BaseDecodeHead):
    ...
    Args:
        ...
        ops_cfg (ConfigType): Configs for OPS-SideAdapterNetwork module
    ...
    def __init__(self, num_classes: int, ops_cfg: ConfigType,
        ...
        super().__init__(
            in_channels=ops_cfg.in_channels,
            channels=ops_cfg.embed_dims,
            num_classes=num_classes,
            **kwargs)
        assert ops_cfg.num_queries == maskgen_cfg.sos_token_num, \
            'num_queries in ops_cfg should be equal to sos_token_num ' \
            'in maskgen_cfg'
        del self.conv_seg
        ...
        if train_cfg:
            self.match_masks = MatchMasks(
                ...
                num_queries=ops_cfg.num_queries,
                num_classes=num_classes,
    ...
    def forward(self, inputs: Tuple[Tensor],
        ...
        Returns:
            mask_props (List[Tensor]): Mask proposals predicted by OPS.
